...
songListContainer.addEventListener('mouseover', function(event) {
    ...
    if (event.target.parentElement.className === 'album-view-song-item') {
        ...
        var songType = getSongType(event.target);
    
    if (songType.getAttribute('data-song-number') != currentlyPlayingSong) {
        songType.innerHTML = playButtonTemplate;
      }
    }
});
...
for (var i = 0; i < songRows.length; i++) {
    ...
    songRows[i].addEventListener('mouseleave', function(event) {
      var songsItems = getSongItems(event.target);
        var songItemNumbers = songItems.getAttribute('data-song-number');
 
      if (songItemNumbers !== currentlyPlayingSong) {
          songItems.innerHTML = songItemNumbers;
     }
  });
}
...
var newSong = function(element) {
    switch (element.className) {
        ...
        case 'ion-play':
        case 'ion-pause':
            return parentName(element, 'song-item-number');
        case 'album-view-song-item':
            return element.querySelector('.song-item-number');
        case 'song-item-title':
        case 'song-item-duration':
            return parentName(element, 'album-view-song-item').querySelector('.song-item-number');
